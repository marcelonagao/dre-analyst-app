...
function GraficoCanaisPorPlataforma({ historico }) {
  ...
  const pointsShopee = historico.map((d, i) => ({ x: paddingX + (i * (svgWidth - 2 * paddingX)) / (historico.length - 1), y: getY(d.shopee || 0) }));
  const pointsMeli = historico.map((d, i) => ({ x: paddingX + (i * (svgWidth - 2 * paddingX)) / (historico.length - 1), y: getY(d.meli || 0) }));
  const pointsExterna = historico.map((d, i) => ({ x: paddingX + (i * (svgWidth - 2 * paddingX)) / (historico.length - 1), y: getY(d.externa || 0) }));
