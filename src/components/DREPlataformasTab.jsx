...
// 👇 IMPORTAÇÃO CORRIGIDA AQUI (Adicionado IconBrain e IconPackage)
import { IconStore, IconChevronDown, IconChevronRight, IconAlertTriangle, IconBrain, IconPackage } from './Icons';
    ...
    <div className="space-y-6 w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full items-start">
        {dre.map((plat, idx) => {
          ...
          // Pega os produtos específicos deste canal
            ...
            <div key={idx} className={`bg-white rounded-2xl shadow-sm border transition-all duration-300 flex flex-col ${isExpanded ? 'border-emerald-400 shadow-emerald-100 lg:col-span-2' : 'border-slate-200/80'}`}>
              <div onClick={() => setExpandedChannel(isExpanded ? null : plat.plataforma)} className="p-6 cursor-pointer hover:bg-slate-50/50 transition-colors">
                <div className="flex justify-between items-center border-b border-slate-100 pb-4 mb-4">
                  <div className="flex items-center space-x-3">
                    <div className={`p-3 rounded-2xl text-white ${plat.plataforma.includes('Shopee') ? 'bg-orange-500' : plat.plataforma.includes('Mercado Livre') ? 'bg-amber-500' : 'bg-blue-500'}`}>
              ...
              {isExpanded && (
                <div className="p-6 border-t border-slate-100 bg-slate-50/30 animate-fadeIn space-y-4">
                  <DeepDiveCanal plataforma={plat.plataforma} margemAtual={plat.margemLiquida} produtos={prodsCanal} topProdutos={topProdutos} pioresProdutos={pioresProdutos} />
                </div>
              )}
            </div>
          );
...
// ==========================================
function DeepDiveCanal({ plataforma, margemAtual, produtos, topProdutos, pioresProdutos }) {
  const [abaInterna, setAbaInterna] = useState('produtos'); 
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="pt-2">
        {abaInterna === 'produtos' && (
          <div className="space-y-4">
            <p className="text-xs text-slate-500">Análise de Margem dos produtos vendidos <strong>exclusivamente na {plataforma}</strong>.</p>
